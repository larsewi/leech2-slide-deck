...
\end{table}
\begin{example}
A \texttt{delete} followed by an \texttt{insert} cancels out — the net effect is
no change at all. However, if the inserted value differs from the deleted one,
the pair collapses into a single \texttt{update} from the deleted value to the
inserted one.
\end{example}
\end{frame}
